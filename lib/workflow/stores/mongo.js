...
function mongoStoreFactory(waterline, Promise, Constants, Errors, assert, _, sanitizer) {
    ...
    */
    ...
    /**
     * Checkout a ready task 
     */
    exports.checkoutReadyTask = function(domain, taskRunnerId, graphId) {
        assert.string(domain, 'domain');
        if (graphId) {
            assert.uuid(graphId, 'graphId');
        }
        ...
        .tap(function(obj) {
            if(obj) {
                var timestamp = new Date();
                var queryGraph = {
                    instanceId: obj.graphId
                };
                var updateTaskGraph = {
                    $set: {}
                };
                var timeKey = ['tasks', obj.taskId, 'taskStartTime'].join('.');
                updateTaskGraph.$set[timeKey] = timestamp;

                return waterline.graphobjects.findAndModifyMongo(queryGraph, {}, updateTaskGraph, options)
                .tap(function(graph){
                    exports.publishGraphRecord(graph);
                });
            }
    ...
    exports.updatePollerStatus = function(id, obj) {
        ...
        if( obj.status !== Constants.Task.States.Succeeded ) {
            console.trace('incrementing failure count');
